...
class Ass(AutoSlots):
    ...
    def __init__(
        ...
        self._sections = {m.group(0): _Section(m.group(0), *m.span(0)) for m in re.finditer(r"(^\[[^\]]*])", lines_file, re.MULTILINE)}
        ...
        for preline, curline, postline in (
            zline for liness in lines_by_styles.values() for zline in zip_offset(liness, liness, liness, offsets=(-1, 0, 1), longest=True)
        ):
            if not curline:
                continue
    ...
    def clean_styles(self) -> None:
        ...
        self.styles = [
            self.styles_map[sname] for sname in OrderedSet(line.style.name for line in self.lines) & OrderedSet(s.name for s in self.styles)
        ]
    ...
    def add_line(self, line: Line, fix_timestamps: bool | None = None) -> None:
        ...
        :param fix_timestamps:      If True, will fix the timestamps on their real start and end time.
                                    If False, start and end times will just be the raw timestamps.
        """
        self._output_lines.append(line.as_text(fix_timestamps=fix_timestamps if fix_timestamps is not None else self._fix_timestamps))
    ...
    def save(
        ...
        with open(self._output, "w", encoding="utf-8-sig") as f:
            ...
            f.write("[Script Info]\n")
            try:
                si_txt = self.meta.script_info.as_text([f"; Script generated by Pyonfx {__version__}\n; https://github.com/Ichunjo/PyonFX"])
            except AttributeError:
                si_txt = ""
            f.write(si_txt + "\n")
            ...
            if lines:
                f.writelines(
                    line.as_text(fix_timestamps=fix_timestamps if fix_timestamps is not None else self._fix_timestamps) for line in lines
                )
            f.write("\n")
...
class AssUntitled(Ass):
    def __init__(self, output: AnyPath | None = None, /, fps: float | None = 24000 / 1001, fix_timestamps: bool = True) -> None:
        """
        :param output:              Output file path
        :param fps:                 Framerate Per Second of the video related to the .ass file
                                    If False, start and end times will just be the raw timestamps.
...
class _MetaData(_DataCore, empty_slots=True):
    ...
    def from_text(cls, text: str) -> Self:
        ...
        for k, v in (
            (m.groupdict()["name"], m.groupdict()["value"]) for m in re.finditer(r"^(?P<name>.*): (?P<value>.*)$", text, re.MULTILINE)
        ):
...
class Line(_AssText, slots_ex=True, slots_ex_exclude="tags"):
    ...
    def add_syls(self, font: Font, vertical_kanji: bool = False) -> None:
        ...
        for si, (k0, k1) in enumerate(zip_offset(ks, ks, offsets=(0, 1), longest=True)):
            ...
            for ptag in (ptag for tagspos in ("pretags", "posttags") for ptag in slash.split(k0.groupdict()[tagspos].replace("}{", ""))):
                if ptag.startswith("\\-"):
